...
BEGIN
    ...
    SELECT horas_extra from calcular_horas_extra(p_user_nombre) INTO v_horas_extra;

    -- Obtener el codigo del personal
    SELECT fk_personal
    INTO v_per_cod
    FROM usuario
    ...
    WHERE pc.fk_personal = v_per_cod and pc.fk_cargo = car.car_cod;

    -- obtener el codigo de su asignacion
    SELECT asig_cod
    INTO v_asig_cod
    FROM asignacion
    WHERE fk_personal = v_per_cod;
    ...
    RETURN QUERY SELECT v_per_cod, total_nomina, v_asistencias, v_sueldo_base, v_sueldo_hora_extra, v_horas_extra;
